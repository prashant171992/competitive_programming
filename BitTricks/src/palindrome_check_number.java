...
public class palindrome_check_number {
    ...
    private boolean isPalindrome(long input) {
        ...
        long msd = 0;
        long lsd = 0;
        int numOfDigits = (int) Math.floor(Math.log10(input)) + 1;
        for (int i = 0; i < (numOfDigits/2); ++i){
            lsd = input % 10;
            msd = input / (long) Math.pow(10, (double) numOfDigits - 1);
            input %= (long) Math.pow(10, (double) numOfDigits - 1);
            input /=10;
            numOfDigits--;
            if (lsd != msd) {
                return false;
    ...
    public static void main(String[] args) throws Exception {
        new palindrome_check_number().run();
    }
