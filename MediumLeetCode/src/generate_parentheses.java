...
public class generate_parentheses {
    ...
    public void backTrack(List<String> answer, String s, int startBracket, int endBracket, int max) {
        if (s.length() == max * 2) {
            answer.add(s);
            return;
        }
        if (startBracket < max){
            backTrack(answer, s+"(", startBracket+1, endBracket, max);
        }
        if (endBracket < startBracket) {
            backTrack(answer, s+")", startBracket, endBracket+1, max);
        }
    }
