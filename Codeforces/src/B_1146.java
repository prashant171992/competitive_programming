...
public class B_1146 {
    ...
    void solve() {
        ...
        for (Character character : input.toCharArray()) {
            if (character != 'a') {countOtherCharacters++;}
            else countOfA++;
        }

        // If other character count is odd
        if (countOtherCharacters % 2 == 1) {
            out.println(":(");
            return;
        }

        String temp = input.substring(0, input.length() - (countOtherCharacters/2));

        StringBuilder checkString = new StringBuilder(temp);

        for (Character character : temp.toCharArray()) {
            if (character != 'a') checkString.append(character);
        }
