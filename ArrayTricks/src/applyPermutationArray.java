...
public class applyPermutationArray {
    ...
    void solve() {
        out.println(applyPermutation(Arrays.asList(1, 2, 3, 4), Arrays.asList(3, 1, 2, 0)));
    }

    private List<Integer> applyPermutation(List<Integer> integerList, List<Integer> permutation) {
        for (int i = 0; i < integerList.size(); ++i) {
            int next = i;

            while (permutation.get(next) >= 0) {
                Collections.swap(integerList, i, permutation.get(next));
                int t = permutation.get(next);
