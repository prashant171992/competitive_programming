...
public class kth_permutation_sequence {
    ...
    void solve() {
        out.println(computeNthPermutation(new ArrayList<Integer>(Arrays.asList(1, 2, 3, 4)), 14));
    }

    private List<Integer> computeNthPermutation(List<Integer> input, int k) {
        List<Integer> result = new ArrayList<>();
        int[] factorial = new int[input.size()+1];
        factorial[0] = 1;
        for (int i = 1; i <= input.size(); ++i) {
            factorial[i] = factorial[i-1] * i;
        }

        k--; // since its 0 based index

        int inputSize = input.size();

        for (int i = 1; i <= inputSize; ++i) {
            int index = k / factorial[inputSize-i];
            result.add(input.get(index));
            input.remove(index);
            k-= index * factorial[inputSize-i];
        }
        return result;
    }
    ...
    public static void main(String[] args) throws Exception {
        new kth_permutation_sequence().run();
    }
