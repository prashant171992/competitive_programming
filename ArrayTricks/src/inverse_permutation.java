...
public class inverse_permutation {
    ...
    void solve() {
        out.println(inversePermutation(Arrays.asList(1, 2, 3, 5, 4)));
    }

    private List<Integer> inversePermutation(List<Integer> list) {
        int temp = -1;
        for (int i = 0; i < list.size(); ++i) {
            ...
            if (temp == -1) {
                temp = list.get(list.get(i)-1);
                list.set(list.get(i)-1, i-list.size()+1);
            } else {
                int ff = list.get(temp-1);
                list.set(temp-1, i-list.size()+1);
                temp = ff;
            }
        }

        for (int i = 0; i < list.size(); ++i) list.set(i, list.get(i) + list.size());
