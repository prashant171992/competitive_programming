...
public class bipartite_matching {
    ...
    class Graph {
        ...
        boolean bipartiteUtil(int vertex) {
            Queue<Integer> queue = new LinkedList<>();
            queue.add(vertex);
            while (!queue.isEmpty()) {
                Integer element = queue.poll();
                for (int i = 0; i < adj[element].size(); ++i) {
                    Integer neighbour = adj[element].get(i);
        ...
        boolean bipartiteMatching() {
            for (int i = 0; i < vertices; ++i) {
                if (color[i] == -1) {
                    color[i] = 1;
                    if (!bipartiteUtil(i)) {
                        out.println("Bipartite matching not possible");
                        return false;
                    }
                }
            }
            out.println("Bipartite matching present");
            return true;
