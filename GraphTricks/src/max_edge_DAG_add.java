...
public class max_edge_DAG_add {
    ...
    class Graph {
        ...
        void addEdgeDAG(List<Integer> list) {
            boolean[] isNeighbour;
            for (int i = 0; i < list.size(); ++i) {
                isNeighbour = new boolean[list.size()];
                for (int j = 0 ; j < adjVertices.get(list.get(i)).size(); ++j) {
                    isNeighbour[adjVertices.get(list.get(i)).get(j)] = true;
                }
                for (int j = i+1; j < list.size(); ++j) {
                    if (!isNeighbour[list.get(j)]) {
    ...
    void solve() {
        ...
        g.addEdgeDAG(g.topologicalSort());
    }
