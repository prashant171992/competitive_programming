...
public class node_level_tree {
    ...
    class Graph {
        int vertexCount;
        List<List<Integer>> adjVertices;
        boolean visited[];
        int level[];

        Graph(int v) {
            this.vertexCount = v;
            this.level = new int[vertexCount+1];
            adjVertices = new ArrayList<>();
            IntStream.range(0, v)
                    .forEach(i -> adjVertices.add(i, new ArrayList<>()));
        ...
        void BFSUtil(int node) {
            ...
            level[node] = 0;
            while (!queue.isEmpty()) {
                Integer currElement = ((LinkedList<Integer>) queue).pop();
                for (int i = 0 ; i < adjVertices.get(currElement).size(); ++i) {
                    int neighbour = adjVertices.get(currElement).get(i);
