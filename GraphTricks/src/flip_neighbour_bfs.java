...
public class flip_neighbour_bfs {
    InputStream is;
    ...
    void solve() {
        ...
        neighbours = flipNeighbour(neighbours, new Coordinate(0, 0));

        for (List<Boolean> list : neighbours) {
            for (Boolean b : list) {
                out.print(b + " ");
            }
            out.println();
    ...
    private List<List<Boolean>> flipNeighbour(List<List<Boolean>> input, Coordinate coordinate) {
        final int[][] SHIFT = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
        Queue<Coordinate> queue = new LinkedList<>();
        queue.add(coordinate);
        while (!queue.isEmpty()) {
            Coordinate queueCoordinate = queue.poll();
            input.get(queueCoordinate.x).set(queueCoordinate.y, !input.get(queueCoordinate.x).get(queueCoordinate.y));
            for (int[] shift : SHIFT) {
                Coordinate next = new Coordinate(queueCoordinate.x + shift[0], queueCoordinate.y + shift[1]);
                if (next.x >= 0 && next.x < input.size() && next.y >= 0 && next.y < input.get(next.x).size()
                        && input.get(next.x).get(next.y) != input.get(coordinate.x).get(coordinate.y)) {
                    queue.add(next);
                }
            }
        }
