...
public class corner_rectangles_squares {
    InputStream is;
    ...
    void solve() {
        ...
        out.println("Rectangles : " + countCornerRectangles(grid));
        out.println("Squares : " + countCornerSquares(grid));

    }

    int countCornerSquares(int[][] grid) {
        int result = 0;
        for (int i = 0; i < grid.length; ++i) {
            for (int j = i + 1; j < grid.length; ++j) {
                int count = 0;
                for (int k = 0; k < grid[0].length; ++k) {
                    if ((grid[i][k] == grid[j][k] && grid[i][k] == 1) && (k+j-i < grid[0].length) && (grid[i][k+j-i] == grid[j][k+j-i] && grid[i][k+j-i] == 1)) {
                        count++;
                    }
                }
                result+= count;
    ...
    public static void main(String[] args) throws Exception {
        new corner_rectangles_squares().run();
    }
