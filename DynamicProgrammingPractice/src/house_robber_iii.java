...
public class house_robber_iii {
    ...
    public int rob(TreeNode root) {
        int[] result = robStep(root);
        return Math.max(result[0], result[1]);
    }

    public int[] robStep(TreeNode treeNode) {
        if (treeNode == null) {
            return new int[2];
        }

        int[] left = robStep(treeNode.left);
        int[] right = robStep(treeNode.right);

        int [] result = new int[2];
        result[0] = Math.max(left[0], left[1]) + Math.max(right[0], right[1]);
        result[1] = treeNode.val + left[0] + right[0];
        return result;
    }
