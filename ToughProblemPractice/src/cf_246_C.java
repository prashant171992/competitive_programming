...
public class cf_246_C {
    ...
    void solve() {
        int n = ni();
        int m = ni();
        int beauty[] = new int[100];
        for (int i = 0; i < n; ++i) {
            beauty[i] = ni();
        }
        int counter = 0;
        List<Integer> array = Arrays.stream(beauty).boxed().collect(Collectors.toList());
        array.sort(Comparator.reverseOrder());
        ArrayList<Integer> initial = new ArrayList<>();
        int start = 0;
        while (counter < m) {
            for (int i = start; i < n && counter < m; ++i) {
                System.out.print(initial.size() + 1 + " ");
                initial.forEach(j -> System.out.print(j + " "));
                System.out.println(array.get(i));
                counter++;
            }
            start++;
            initial.clear();
            IntStream.range(0, start).forEach(j -> initial.add(array.get(j)));
        }
    }
