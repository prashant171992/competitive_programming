...
public class keys_and_rooms {
    ...
    class Graph {
        ...
        public Graph(int vertex) {
            this.vertex = vertex;
            this.neighbour = new List[vertex];
    ...
    public boolean canVisitAllRooms(List<List<Integer>> rooms) {
        ...
        for (int i = 0; i < rooms.size(); ++i) {
            for (int j = 0; j < rooms.get(i).size(); ++j) {
                graph.neighbour[i].add(rooms.get(i).get(j));
            }
        }

        boolean visited[] = new boolean[rooms.size()];
        DFSUtil(visited, 0, graph);
        for (int i = 0; i < rooms.size(); ++i) {
            if (!visited[i]) {
                return false;
            }
    ...
    void DFSUtil(boolean visited[], int room, Graph graph) {
        visited[room] = true;
        for (int i = 0; i < graph.neighbour[room].size(); ++i) {
            int neigbour = graph.neighbour[room].get(i);
            if (!visited[neigbour]) {
                DFSUtil(visited, neigbour, graph);
            }
        }
        return;
    }
